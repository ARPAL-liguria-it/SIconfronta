...
serie non sono disponili i valori delle singole misure ma bensì solo la
...
risultati di una serie di misure sono stati confrontanti con le
prestazioni riportate in un metodo di riferimento.

Sul metodo di riferimento viene riportata la media, la deviazione
standard, rispettivamente pari a 2.08 mg/kg, 0.17 mg/kg per 20 misure
...
L'operatore Pasquale Dinatale vuole verificare che i due strumenti con
...
Pasquale non si può permettere di fermare le due linee di analisi per
...
Pasquale, che queste cose le sa, calcola la differenza tra le due serie
...
$A - B < 0$, possiamo concludere che lo strumento B produca valori di
concentrazione sovrastimati rispetto a quelli prodotti dallo strumento
A.
...
::: {.minipage style="border:5px solid #2780E3; border-radius: 12px; width: 80%; margin: 25px; padding: 10px"}
In questa opzione, la media di una serie di misure viene paragonata a un
valore di deviazione standard noto, privo quindi di incertezza.
...
Tra i due comuni di Acqualercia Soprana e Acqualercia Sottana infuria
una disputa su chi abbia l'acqua più buona.

Il laboratorio di analisi chimiche di Capracotta superiore si offre di
...
| parametro | acqualercia | valore | incertezza |
|-----------|-------------|--------|------------|
...
1.  Nella scheda *Scopo* di SIconfronta si seleziona l'ultima opzione;
2.  nella scheda *Dati* si seleziona il file con la tabella dei dati in
    formato csv. Si identificano le colonne relative alle informazioni
    sugli analiti, gruppo, risposte e incertezza estese rispettivamente
    come "parametro", "acqualercia", "valore" e "incertezza".
...
piombo misurati nel campione prelevato in Acqualercia Sottana, risultano
maggiori di quelli misurati nel campione prelevato nel comune di
Acqualercia Soprana ( $E_n$ pari rispettivamente a 1.167 e 2.230).
