...
testServer(
  mod_compare034_1sample_sigma_server,
  ...
  args = list(r), {
    ...
    expect_true(inherits(output$chitest, "character"))
    ...
    expect_named(r$compare03x,
                c("ftest", "ttest", "significance", "alternative", "outliers",
                  ...
                  "label2", "sd2", "ready", "click"),
                ignore.order = TRUE)
    expect_length(names(r$compare03x), 14)
})

test_that("module compare034 for 1 sample input ui works", {
  ui <- mod_compare034_1sample_sigma_inputs_ui(id = "test")
  golem::expect_shinytaglist(ui)
  # Check that formals have not been removed
  fmls <- formals(mod_compare034_1sample_sigma_inputs_ui)
  for (i in c("id")){
    expect_true(i %in% names(fmls))
  }
})

test_that("module compare034 for 1 samples output ui works", {
  ui <- mod_compare034_1sample_sigma_output_ui(id = "test")
  golem::expect_shinytaglist(ui)
  # Check that formals have not been removed
  fmls <- formals(mod_compare034_1sample_sigma_output_ui)
  for (i in c("id")){
    expect_true(i %in% names(fmls))
  }
})
