...
testServer(
  mod_compare032_2samples_par_server,
...
test_that("module compare031 for 2 samples input ui works", {
  ui <- mod_compare032_2samples_par_inputs_ui(id = "test")
  golem::expect_shinytaglist(ui)
  # Check that formals have not been removed
  fmls <- formals(mod_compare032_2samples_par_inputs_ui)
  for (i in c("id")){
...
test_that("module compare031 for 2 samples output ui works", {
  ui <- mod_compare032_2samples_par_output_ui(id = "test")
  golem::expect_shinytaglist(ui)
  # Check that formals have not been removed
  fmls <- formals(mod_compare032_2samples_par_output_ui)
  for (i in c("id")){
    expect_true(i %in% names(fmls))
  }
})
