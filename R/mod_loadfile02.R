...
mod_loadfile02_server <- function(id, r) {
  moduleServer(id, function(input, output, session) {
    ...
    msg_waiting <- reactive({
      ...
      switch(
        r$aim01$aim,
        "2samples" = includeMarkdown(system.file("rmd", "msg_2samples.Rmd", package = "comparat")),
        "2samples_par" = includeMarkdown(system.file("rmd", "msg_2samples_par.Rmd", package = "comparat")),
        "1sample_mu" = includeMarkdown(system.file("rmd", "msg_1sample_mu.Rmd", package = "comparat")),
        "1sample_sigma" = includeMarkdown(system.file("rmd", "msg_1sample_sigma.Rmd", package = "comparat")),
        "2values_unc" = includeMarkdown(system.file("rmd", "msg_2values_unc.Rmd", package = "comparat"))
      )
    })
    ...
    sumgroup <- reactive({
      ...
      dataframe()[[input$groupvar]] |> unique() |> length()
    })
    ...
    ## checking that data is properly formatted
    dataok <- reactive({
      ifelse(sumchar() == 2 & sumnum() == reqsumnum() & sumgroup() == reqsumgroup(), TRUE, FALSE)
    })
    ...
    is2values <- reactive({
      ...
      ifelse(!isTRUE(dataok()), "",
             ifelse(r$aim == "2values_unc", "2values", "not_2values")
      )
    })
    ...
    output$datatable <- DT::renderDT({
      validate(
        ...
        need(sumgroup() == reqsumgroup(),
             message = sprintf(
               "Hai fornito un dataset con %s grupp%s ma ne %s.",
               sumgroup(),
               ifelse(sumgroup() == 1, "o", "i"),
