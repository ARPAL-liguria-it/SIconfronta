...
mod_compare032_2samples_par_output_ui <- function(id) {
  ...
  tagList(
    ...
    tabsetPanel(
      ...
      tabPanel("help",
               includeMarkdown(system.file("rmd", "help_compare032_2samples_par.Rmd", package = "comparat"))),
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(r$compare03$myparameter, ignoreNULL = FALSE, {
      ...
      if(r$compare03[[r$compare03$myparameter]]$saved |> isTRUE()){
        ...
        updateNumericInput(session,
                           "n",
                           value = r$compare03[[r$compare03$myparameter]]$n2)
    ...
    mean_a <- reactive({
      ...
      selected_data()$response |> mean()
    })

    sd_a <- reactive({
      req(selected_data())

      selected_data()$response |> stats::sd()
    })

    n_a <- reactive({
      req(selected_data())

      selected_data()$response |> length()
    })
    ...
    mysummary <- reactive({
      ...
      res <- data.frame(c("mean", "sd", "n"),
                        c(mean_a(), sd_a(), n_a()),
                        c(r$compare03x$mean2, r$compare03x$sd2, r$compare03x$mean2))
      colnames(res) <- c("index", label_a(), r$compare03x$label2)
      res
    })
    ...
    summarytable <- reactive({
      ...
      rowsummary_2samples_par(
        data = selected_data(),
        group = "group",
        group2 = r$compare03x$label2,
        mean2 = r$compare03x$mean2,
        sd2 = r$compare03x$sd2,
        n2 = r$compare03x$n2,
        response = "response",
        udm = r$compare03x$udm
      )
...
\u21e8 %s"
    ...
    observeEvent(ttest_html(), {
      ...
      r$compare03x$ggsummary <- mysummary()
